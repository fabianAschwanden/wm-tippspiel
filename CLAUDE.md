...
Mehrspieler: Registrierung/Login per E-Mail (ohne Passwort, Demo), Session-Cookie; Postgres async via DATABASE_URL (Neon auf Vercel, Pool aus @neondatabase/serverless) bzw. PGlite in .data/pglite ohne DATABASE_URL — Tests: PGLITE_DATA_DIR=memory:// (lib/server/db.ts, lib/server/auth.ts). API unter app/api/ (auth, tips, leaderboard, matches, live, cron), Client-Helfer lib/api.ts + lib/hooks.ts. Tipps bis Anstoss änderbar (Server prüft, 422), fremde Tipps erst nach Anstoss einsehbar.
